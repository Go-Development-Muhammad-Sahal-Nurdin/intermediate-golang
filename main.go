...
func getAllMahasiswa(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		err := rows.Scan(&mahasiswa.Id, &mahasiswa.Npm, &mahasiswa.Nama, 
		&mahasiswa.Kelas, &mahasiswa.Profile)
...
func getMahasiswaByName(w http.ResponseWriter, r *http.Request) {
	...
	defer db.Close()
	params := mux.Vars(r)
	query := fmt.Sprintf("SELECT * FROM mahasiswa WHERE nama LIKE '%s%%'", 
	params["keyword"])
	...
	for rows.Next() {
		if err := rows.Scan(&mahasiswa.Id, &mahasiswa.Npm, &mahasiswa.Nama,
		&mahasiswa.Kelas, &mahasiswa.Profile); err != nil {
			log.Print(err.Error())
		}
		mhs = append(mhs, mahasiswa)
	}
...
func main() {
	...
	r.HandleFunc("/mahasiswa/search/{keyword}", 
	getMahasiswaByName).Methods("GET")
	...
	log.Print("Server berjalan di http://localhost:8075")
	srv.ListenAndServe()
}